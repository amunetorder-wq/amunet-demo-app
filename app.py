...
with tab4:
    st.markdown("### 「従業員のセキュリティ意識を劇的に向上。クラウド型訓練システム」")
    st.write("Google Cloud (GCP) 上にオープンソースの『Gophish』を構築。自社の従業員に対して安全な疑似フィッシングメールを送信し、セキュリティ意識を測定・向上させます。")
    
    st.markdown("""
    | セキュリティの課題 | 訓練システムによる解決策 |
    | :--- | :--- |
    | 従業員が怪しいメールを開いてしまうか不安 | 実際の業務を模した安全な「疑似攻撃メール」を送信テスト |
    | 誰がリンクをクリックしたか分からない | 「誰が」「いつ」メールを開封・クリックしたかをダッシュボードで可視化 |
    | 教育システムを導入すると高額になる | オープンソース（Gophish）＋クラウド（GCP）活用で低コストに構築 |
    """)
    
    st.info("💡 **リアルな訓練**: 実際の取引先や社内システムを装った、巧妙なテストメールを作成可能です。\n\n💡 **結果のレポート化**: 訓練終了後は、開封率やクリック率をグラフ化したレポートを自動生成し、社内のセキュリティ教育に直結させます。")

    st.markdown('**【システム画面イメージ】**')
    st.write("管理者用ダッシュボード（デモデータ）")
    
    # Gophishのダッシュボード結果を模したテーブル表示
    gophish_data = {
        "訓練キャンペーン名": ["2026年 標的型攻撃訓練 (秋季)"],
        "送信対象者数": ["50 名"],
        "メール開封率": ["42% (21名)"],
        "リンククリック率": ["12% (6名)"],
        "データ入力率": ["4% (2名)"]
    }
    st.table(pd.DataFrame(gophish_data))
    
    if st.button("訓練結果レポート（PDF）を生成（デモ）", use_container_width=True):
        with st.spinner("レポートを作成中..."):
            time.sleep(1.5)
        st.success("✅ 訓練結果の集計が完了しました。（※本来はここでPDFがダウンロードされます）")